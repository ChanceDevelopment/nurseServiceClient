...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (view) {
        ...
        view.frame = frame;
        CGFloat labelW = 200-100;
        CGFloat labelH = 20;
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10+50, (height-labelH)/2, labelW, labelH)];
        label.text = title;
...
- (NSString *)invoke_titleImageForHeaderInSection:(NSInteger)section {
    if (self.datasource&&[self.datasource respondsToSelector:@selector(mTableView:titleImageForHeaderInSection:)]) {
        return [self.datasource mTableView:self titleImageForHeaderInSection:section];
    }
    return @"";
}
